...
class BaseModel(pybamm.BaseBatteryModel):
    ...
    def __init__(self, options=None, name="Unnamed sodium-ion model", build=False):
        super().__init__(options, name)
        self.param = pybamm.SodiumIonParameters(self.options)
    ...
    def set_total_interface_submodel(self):
        self.submodels["total interface"] = pybamm.interface.TotalInterfacialCurrent(
            self.param, "sodium-ion", self.options
        )
    ...
    def set_li_metal_counter_electrode_submodels(self):
        for domain in ["negative", "positive"]:
            ...
            if (
                ...
                ] = pybamm.kinetics.kineticInverseButlerVolmer(
