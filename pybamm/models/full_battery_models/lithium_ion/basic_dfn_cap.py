...
class BasicDFNCap(BaseModel):
    ...
    def __init__(self, name="Doyle-Fuller-Newman model"):
        ...
        delta_phi = pybamm.Variable("Potential difference [V]")
        ...
        # eta_p = phi_s - phi_e - U but Up = 0 anyways  
        # eta_p = phi_s_p - phi_e_p 
        # Feta_RT_p = param.F * eta_p / (param.R * T)
       
        # jp is sum of faradic current and intercalation current 
        # it is assumed that no intercalation occurs in positive electrode 
        # the charge stored in AC electrode is due to double layer alone.
        # j_p = jpi + jpf
        # jpi = 0 
        # jp = jpf 
        # j_p = 2 * j0_p * pybamm.sinh(param.p.prim.ne / 2 * Feta_RT_p)
        # double layer capacitance equation
        delta_phi = phi_s_p - phi_e_p
        C_dl = param.p.C_dl
        j_p = C_dl * self.rhs[delta_phi]
        self.initial_conditions[delta_phi] = pybamm.Scalar(3.14)
        # self.rhs[delta_phi] = j_p/C_dl
        ...
        # no spatial gradients at popsitive elctrode 
        N_s_p = 0 
        ...
        self.initial_conditions[phi_s_p] = pybamm.Scalar(0.5264)
