...
class BasicSPMe(BaseModel):
    ...
    def __init__(self, name="SPMe model"):
        ...
        # phi_s_n_x = (param.n.prim.U(sto_surf_n, T)
        #             - (i_boundary_cc / sigma_eff_n) * (
        #                  (2*L_n - x_n_edge) * x_n_edge / (2*L_n)
        #             ) 
        #             + i_boundary_cc*L_n / (3*sigma_eff_n)
        #             - (1/L_n)*pybamm.Integral(delta_phi_e_n, x_n)
        #             + pybamm.x_average(eta_c_n)
        #             + eta_n_avg
        # )
        
        # phi_s_p_x = ( param.p.prim.U(sto_surf_p, T)
        #             + (i_boundary_cc / sigma_eff_p) * (
        #             2*(L_x - L_p) - x_p_edge) * x_p_edge / (2*L_p)
        #             - (i_boundary_cc / sigma_eff_p) * (
        #                 2 * L_p**2 - 6*L_p + 3
        #             ) / (6*L_p)
        #             - (1/L_p)*pybamm.Integral(delta_phi_e_p, x_p)
        ...
        phi_s_n_x = -1*(param.n.prim.U(sto_surf_n, T)
                    - (i_boundary_cc / sigma_eff_n) * (
                         (2*L_n - x_n) * x_n / (2*L_n)
                    ) 
                    + i_boundary_cc*L_n / (3*sigma_eff_n)
        ...
        # phi_s_p_L = 0
        phi_s_p_L = pybamm.boundary_value(phi_s_p_x, "right")
        phi_s_n_0 = pybamm.boundary_value(phi_s_n_x, "left")
        # phi_s_n_0 = 0
        
        V = phi_s_p_L - phi_s_n_0 - param.R_contact*i_cell*param.A_cc
