...
class BasicSPMe(BaseModel):
    ...
    def __init__(self, name="SPMe model"):
        ...
        tor = pybamm.concatenation(
            eps_nb**param.n.b_e, eps_sb**param.s.b_e, eps_pb**param.p.b_e
        )
        ...
        # i_app = i_cell
        # electrolyte current
        # i_e_n_edge = i_app * x_n_edge / L_n
        # i_e_s_edge = pybamm.PrimaryBroadcastToEdges(i_app, "separator")
        # i_e_p_edge = i_app * (L_x - x_p_edge) / L_p
        
        # i_e_n = i_app * x_n / L_n
        # i_e_s = pybamm.PrimaryBroadcast(i_app, "separator")
        # i_e_p = i_app * (L_x - x_p) / L_p
        ...
        # indef_integral_n = pybamm.IndefiniteIntegral(
        #     i_e_n / (param.kappa_e(c_e_n, T) * tor_n), x_n
        # )
        # indef_integral_s = pybamm.IndefiniteIntegral(
        #     i_e_s / (param.kappa_e(c_e_s, T) * tor_s), x_s
        # )
        # indef_integral_p = pybamm.IndefiniteIntegral(
        #     i_e_p / (param.kappa_e(c_e_p, T) * tor_p), x_p
        # )
        ...
        # integral_p = indef_integral_p 
        
        # delta_phi_e = - (pybamm.x_average(integral_p) - pybamm.x_average(integral_n))
        # delta_phi_e = - (pybamm.x_average(integral_n))
        # delta_phi_e = 0 
        
        # c_e_s_avg = pybamm.x_average(c_e_s)
        # kappa_e_n = param.kappa_e(c_e_n_avg, T)
        # kappa_e_s = param.kappa_e(c_e_s_avg, T)
        # kappa_e_p = param.kappa_e(c_e_p_avg, T)
        ...
        delta_phi_e_av = (
            - (1/L_p)*pybamm.Integral(delta_phi_e_p, x_p)
            + (1/L_n)*pybamm.Integral(delta_phi_e_n, x_n)
            - pybamm.boundary_value(delta_phi_e_s, "right")
            - pybamm.boundary_value(delta_phi_e_n, "right")
        )
        
        delta_phi_e = delta_phi_e_av
        ...
        # sigma_eff_p = param.p.sigma(T)
        # sigma_eff_n = param.n.sigma(T)
        
        # delta_phi_p_av = delta_phi_e_av
        
        # delta_phi_n_av = delta_phi_e_av
        
        # phi_s_cn =  - param.n.prim.U(sto_surf_n, T)  
        # phi_s_n2 = phi_s_cn + (i_boundary_cc / sigma_eff_n) * (
        #         x_n * (x_n - 2 * L_n) / (2 * L_n)
        #     )
        
        # phi_s_n_av = pybamm.x_average(phi_s_n2)
        ...
        # phi_e_p_av = (1/L_p)*pybamm.Integral(phi_e_p, x_p)
        
        # const = (
        #         delta_phi_p_av
        #         + phi_e_p_av
        #         + (i_boundary_cc / sigma_eff_p) * (L_x - L_p / 3)
        #     )
        
        # phi_s_p2 = const - (i_boundary_cc / sigma_eff_p) * (
        #         x_p + (x_p - L_x) ** 2 / (2 * L_p)
        #     )
        
        # phi_s_cp = pybamm.boundary_value(phi_s_p2, "right")
        # V = phi_s_cp - param.R_contact*i_cell*param.A_cc
        
        # # phi_p3 = (param.p.prim.U(sto_surf_p, T)
        # #           + )
